# modified copy of https://github.com/vimalabs/VIMA/blob/main/vima/policy/vima_policy.py
...
class VIMAMCPolicy(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.obs_fusion_layer = nn.Linear(self.obj_encoder.output_dim, embed_dim)

        self.action_encoder = vnn.ActionEmbedding(
            output_dim=embed_dim,
            embed_dict={
                "cursor_x": vnn.ContinuousActionEmbedding(
                    output_dim=256,
                    input_dim=1,
                    hidden_dim=256,
                    hidden_depth=1,
                ),
                "cursor_y": vnn.ContinuousActionEmbedding(
                    output_dim=256,
                    input_dim=1,
                    hidden_dim=256,
                    hidden_depth=1,
                ),
                #"position": vnn.ContinuousActionEmbedding(
                #    output_dim=256,
                #    input_dim=1,
                #    hidden_dim=256,
                #    hidden_depth=1,
    ...
    def forward_obs_token(self, obs):
        ...
        obj_mask = {
            k: objects["mask"][k].unsqueeze(0) for k in objects["mask"]
        }

        #ee_feats = self.end_effector_encoder(ee)
        #ee_feats = ee_feats.unsqueeze(2).repeat(1, 1, img_feats.shape[-2], 1)

        obs_feats = self.obs_fusion_layer(img_feats.unsqueeze(0))
